...
const ArtCanvas = ({ onSave, initialData = null }) => {
  ...
  const brushTypes = [
    ...
    { id: 'fine', name: 'Fine', icon: Circle, description: 'Fine detail brush' }
  ];
  ...
  const saveTransform = useCallback((oldShapes, newShapes) => {
    const transform = {
      oldShapes: [...oldShapes],
      newShapes: [...newShapes],
      timestamp: Date.now()
  ...
  const drawBrushStroke = (ctx, startPos, endPos, brushType, brushSize, opacity, color) => {
    const alpha = opacity / 100;
    ...
    switch (brushType) {
      case 'pencil':
        ...
        // Add subtle texture with smaller strokes
        if (brushSize > 2) {
          ctx.lineWidth = Math.max(1, brushSize * 0.3);
          ctx.globalAlpha = alpha * 0.4;
          
          for (let i = 0; i < 3; i++) {
            const offset = (Math.random() - 0.5) * brushSize * 0.4;
            ctx.beginPath();
      ...
      default: { // normal brush with watercolor effect
    ...
    if (tool === 'select') {
      ...
      if (clickedShape) {
        ...
        if (handle) {
          ...
          setDragOffset({ x: pos.x - clickedShape.x, y: pos.y - clickedShape.y });
    ...
    if (tool === 'brush' || tool === 'eraser') {
      ...
      if (tool === 'eraser') {
        ...
        ctx.moveTo(pos.x, pos.y);
               ...
               tool === 'triangle' || tool === 'ellipse' || tool === 'star' || tool === 'diamond' || 
  ...
  const draw = (e) => {
    ...
    if (tool === 'brush' || tool === 'eraser') {
      ...
      if (tool === 'eraser') {
        ...
        drawBrushStroke(ctx, lastDrawPos, pos, brushType, brushSize, opacity, color);
        setLastDrawPos(pos);
      }
